        ...
        if self.method in ("ngram", "ngram_gpu"):
            ...
            if self.model is not None:
                ...
                if uses_target_nemotron_h_mtp and self.max_model_len is None:
                    # Nemotron-H MTP is loaded from the target checkpoint and
                    # supports the same context. Preserve an explicit target
                    # max-model-len override instead of re-deriving the native
                    # checkpoint context length for the MTP head.
