...
class Qwen3CoderToolParser(ToolParser):
    ...
    def _tool_allows_empty_arguments(
        ...
        if not func_name or tools is None:
            return False
        param_config = self._get_arguments_config(func_name, tools)
        return isinstance(param_config, dict) and len(param_config) == 0
    ...
    def extract_tool_calls_streaming(
                    ...
                    if self.function_end_token in tool_text:
                        ...
                        if func_content_end != -1:
                            ...
                            try:
                                ...
                                if parsed_tool:
                                    if (
                                        parsed_tool.function.arguments in ("", "{}")
        ...
        if self.in_function:
                ...
                if remaining_args:
                    ...
                    if not self.current_tool_name_sent:
                        ...
                        if tool_call.function is None:
                            tool_call.function = DeltaFunctionCall()
                        tool_call.function.name = self.current_function_name
            ...
            # For auto tool calls, do not stream partial JSON argument
            # fragments before the function body is complete. If the model
            # stops between fragments, clients are left with malformed JSON
            # that cannot be reconstructed safely. Emit the tool only once the
            # closing function tag is present so the arguments are complete.
            return None
