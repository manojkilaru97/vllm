...
from vllm.entrypoints.openai.engine.protocol import DeltaFunctionCall, DeltaMessage, DeltaToolCall, ExtractedToolCallInformation
from vllm.logger import init_logger
from vllm.tokenizers import TokenizerLike
from vllm.tool_parsers.abstract_tool_parser import Tool, ToolParser
from vllm.tool_parsers.utils import UnexpectedAstError, handle_single_tool, make_valid_python

logger = init_logger(__name__)


class SuperV1ToolParser(ToolParser):
    """Parser for BFCL-style tool calls wrapped in <TOOLCALL>...</TOOLCALL>."""
    ...
    def _parse_tool_text(self, text: str):
        normalized = self._normalize_tool_text(text)
        module = ast.parse(normalized)
        parsed = getattr(module.body[0], "value", None)
        if isinstance(parsed, ast.List) and all(isinstance(e, ast.Call) for e in parsed.elts):
            return [handle_single_tool(e) for e in parsed.elts]
        if isinstance(parsed, ast.Call):
            return [handle_single_tool(parsed)]
        raise UnexpectedAstError("Tool output must be a call or list of calls")
    ...
    def extract_tool_calls(
        ...
        try:
            ...
            logger.exception("Error in extracting Super v1 tool call from response.")
            return ExtractedToolCallInformation(
                tools_called=False,
                tool_calls=[],
                content=model_output,
    ...
    def extract_tool_calls_streaming(
        ...
        if end_idx == -1:
            tool_portion = current_text[start_idx + len(self.TOOL_START):]
            if make_valid_python(self._normalize_tool_text(tool_portion)) is not None:
                return None
            return None
        ...
        try:
            ...
            logger.exception("Error while streaming Super v1 tool call.")
            return None
