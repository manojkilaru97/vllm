...
async def run_server_worker(
    ...
    otel_configured = os.getenv("VLLM_LOG_PAYLOADS", "0") == "1" or any(
        os.getenv(name)
        for name in (
                ...
                class OtelLogFilter(logging.Filter):
                    def filter(self, record: logging.LogRecord) -> bool:
                        if "Avg prompt throughput" in record.getMessage():
                            return False
                        return not record.name.startswith("vllm.v1.metrics")

                otel_handler.addFilter(OtelLogFilter())
                vllm_root_logger = logging.getLogger("vllm")
                if not any(
                    isinstance(h, type(otel_handler)) for h in vllm_root_logger.handlers
                ):
            ...
            if meter is not None or os.getenv("OTEL_EXPORTER_OTLP_METRICS_ENDPOINT"):
                ...
                start_prom_to_otel_bridge(
                    ...
                    interval_seconds=float(
                        os.getenv("VLLM_PROM_SCRAPE_INTERVAL", "30")
                    ),
                )
