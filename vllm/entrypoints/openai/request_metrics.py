...
request_type_audio: Counter | None = None
request_type_tool_call: Counter | None = None
request_type_structured_output: Counter | None = None
request_input_images: Counter | None = None
request_input_videos: Counter | None = None
request_input_audios: Counter | None = None
request_input_tools: Counter | None = None
num_aborted_requests: Counter | None = None
...
def register_request_type_counters() -> None:
    global request_type_image, request_type_video, request_type_audio
    global request_type_tool_call, request_type_structured_output
    global request_input_images, request_input_videos, request_input_audios
    global request_input_tools, num_aborted_requests, _counters_registered
    ...
    request_type_audio = Counter(
        name="vllm:request_type_audio_total",
...
def record_aborted_request() -> None:
    if num_aborted_requests is not None:
...
def _record_summary(summary: RequestMetricsSummary) -> None:
    if not _counters_registered:
        return
    if summary.has_images and request_type_image and request_input_images:
        request_type_image.inc()
        request_input_images.inc(summary.image_count)
    if summary.has_videos and request_type_video and request_input_videos:
        request_type_video.inc()
        request_input_videos.inc(summary.video_count)
    if summary.has_audios and request_type_audio and request_input_audios:
        request_type_audio.inc()
        request_input_audios.inc(summary.audio_count)
    if summary.has_tools and request_input_tools:
        request_input_tools.inc(summary.tool_count)
    if summary.has_tool_calls_enabled and request_type_tool_call:
        request_type_tool_call.inc()
    if summary.has_structured_output and request_type_structured_output:
...
def _detect_structured_output_kind(request: Any) -> str | None:
    ...
    if isinstance(structured_outputs, dict):
        ...
        return "structured_outputs" if structured_outputs else None
