...
class OpenAIServingChat(OpenAIServing):
    ...
    def _compute_newline_token_ids(
        ...
        # Method 2: vocab scan for tokens whose decoded form ends with newline.
    ...
    def _inject_think_end_token_id(
        ...
        """Inject think_end_token_id / end_token_ids into extra_args for reasoning budget enforcement."""
        if not self.reasoning_parser:
            return

        # Check if reasoning_budget is set in chat_template_kwargs
        ct_kwargs = request.chat_template_kwargs
        if not isinstance(ct_kwargs, dict):
            return
        rb = ct_kwargs.get("reasoning_budget")
        if rb is None or rb == -1:
            return

        # Get the end token id from the reasoning parser.
        try:
            temp_parser = self.reasoning_parser(  # type: ignore[call-arg]
                tokenizer,
                chat_template_kwargs=ct_kwargs,
            )
            think_end_id = getattr(temp_parser, "end_token_id", None)

            # Fallback for parsers that only expose `end_token` string.
            if think_end_id is None:
                end_token = getattr(temp_parser, "end_token", None)
            ...
            logger.debug(
                "Injected think_end_token_id=%d and newline_token_ids=%s for reasoning budget enforcement",
                think_end_id,
    ...
    async def chat_completion_stream_generator(
        ...
        # Check if thinking is explicitly disabled via chat_template_kwargs
        # If so, we should skip reasoning extraction and go straight to
        # tool parsing (when tools are enabled).
        thinking_disabled = (
        ...
        try:
            if self.reasoning_parser:
                ...
                # If thinking is disabled, mark reasoning as already ended
                # so that tool parsing can proceed immediately.
                if thinking_disabled and reasoning_end_arr is not None:
                ...
                for output in res.outputs:
                    ...
                    # handle streaming deltas for tools with named tool_choice
                        ...
                        if (
                            ...
                            # Note: We don't check prompt_token_ids here because
                            # multi-turn conversations may have </think> from
                            # history assistant messages. The reasoning_parser
                            # handles enable_thinking=False internally.
                        ...
                        # Note: We don't check prompt_token_ids for </think> here
                        # because multi-turn conversations may have </think> from
                        # history assistant messages. The reasoning_parser handles
                        # enable_thinking=False internally.
                    ...
                    # and reasoning parser
                        ...
                        if not reasoning_end_arr[i]:
                            ...
                            if (
                                ...
                                # When encountering think end id in delta_token_ids,
                                # set reasoning status to end. Remove the text and
                                # token ids related to 'reasoning'.
                                if reasoning_parser.is_reasoning_end(output_token_ids):
                                    reasoning_end_arr[i] = True
                                    current_token_ids = (
