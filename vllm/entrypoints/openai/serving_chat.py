...
class OpenAIServingChat(OpenAIServing):
    ...
    async def create_chat_completion(
        ...
        try:
            ...
            # Check for unsupported tool_choice "required"
            if request.tool_choice == "required":
                return self.create_error_response(
                    "tool_choice \"required\" is not supported in this implementation"
                )
                ...
                for output in res.outputs:
                    ...
                    # and reasoning parser - but skip reasoning parser in /no_think mode
                        ...
                        # handle tool calls only after reasoning is done,
                            ...
                            # Even after reasoning ends, we need to pass tokens through
                            # the reasoning parser first for proper buffering/coordination
                            ...
                            # If reasoning parser returned None (buffering), don't call tool parser
                            if reasoning_delta_message is None:
                                delta_message = None
                            else:
                                # If reasoning parser returned content, it should be final content
                                if reasoning_delta_message.content is not None:
                                    delta_message = reasoning_delta_message
                                else:
                                    # Otherwise proceed with tool parsing
                    ...
                    # BUT still send final chunk if model is finished generating
                    if delta_message is None and output.finish_reason is None:
                        continue
                    ...
                    # if the model is finished generating
                        ...
                        # If delta_message is None (e.g. from reasoning parser), create empty delta
                        final_delta = delta_message if delta_message is not None else DeltaMessage(content="")
                        choice_data = ChatCompletionResponseStreamChoice(
                            index=i,
                            delta=final_delta,
                            logprobs=logprobs,
