...
class XgrammarBackend(StructuredOutputBackend):
    ...
    def compile_grammar(
        ...
        *,
        ...
        effective_disable_any_whitespace = (
            self.disable_any_whitespace
            if disable_any_whitespace is None
            else disable_any_whitespace
        )
        if request_type == StructuredOutputOptions.JSON:
            ctx = self.compiler.compile_json_schema(
                grammar_spec, any_whitespace=not effective_disable_any_whitespace
            )
        elif request_type == StructuredOutputOptions.JSON_OBJECT:
            ctx = self.compiler.compile_json_schema(
                '{"type": "object"}',
                any_whitespace=not effective_disable_any_whitespace,
            )
            ...
            ctx = self._compile_structural_tag(grammar_spec)
        else:
            logger.error(
    ...
    def _compile_structural_tag(self, grammar_spec: str):
        ...
        if "structures" in s_tag:
            ...
            grammar = xgr.Grammar.from_structural_tag(grammar_spec)

        # Structural-tag grammars are used for reasoning-aware handoff flows.
        # Reusing the cached structural-tag compiler path has been observed to
        # poison later concurrent requests, so compile them through a fresh
        # no-cache compiler instead.
        uncached_compiler = xgr.GrammarCompiler(
            self.tokenizer_info,
            max_threads=8,
            cache_enabled=False,
        )
        return uncached_compiler.compile_grammar(grammar)
