...
class StructuredOutputManager:
    ...
    def _create_grammar(self, request: "Request") -> StructuredOutputGrammar:
        ...
        try:
            ...
            if isinstance(self.backend, XgrammarBackend):
                logger.warning(
                    "xgrammar compile failed for request %s; "
                    "trying guidance fallback.",
                    getattr(request, "request_id", "<unknown>"),
                    exc_info=True,
                )
                try:
                    ...
                    logger.exception(
                        "Guidance fallback compile also failed for request %s.",
                        getattr(request, "request_id", "<unknown>"),
                    )
            raise
